...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th style="width: 350px;">Serviço</th>
	...
	@if($ordem->imagens->count() > 0)
		...
		<table>
			@foreach($ordem->imagens as $img)
				@php
					$path = public_path('storage/' . $img->arquivo);
				@endphp
				@if(file_exists($path))
				<tr style="vertical-align: top;">
					<td style="width: 120px; padding: 5px;">
						<img src="{{ 'data:image/png;base64,' . base64_encode(file_get_contents($path)) }}"
							alt="Imagem OS"
							style="max-height: 120px; max-width: 100px; border: 1px solid #ccc; padding: 2px;">
					</td>
					<td style="padding: 5px;">
						<p style="font-size: 11px; margin: 0;"><strong>Usuário:</strong> {{ $img->usuario->name ?? 'Desconhecido' }}</p>
						<p style="font-size: 11px; margin: 0;"><strong>Data:</strong> {{ \Carbon\Carbon::parse($img->created_at)->format('d/m/Y H:i') }}</p>
					</td>
				</tr>
				@endif
			@endforeach
