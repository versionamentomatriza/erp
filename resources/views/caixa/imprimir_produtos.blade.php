...
<head>
    ...
    <style type="text/css">
        ...
        *{
		...
		.wrap {
			max-width: 260px;   /* ajuste se quiser mais largo/estreito */
        ...
        <tbody>
			...
			@if($v->tipo !== 'Nfe')
				...
				<td class="b-top wrap">
					@php
						$linhas = [];
						// Percorre itens tanto de NFCE quanto de NFE
						if (isset($v->itens) && count($v->itens)) {
							foreach ($v->itens as $it) {
								// Descrição: tenta método descricao(), depois produto->nome, depois nome do item
								$desc = method_exists($it, 'descricao') ? $it->descricao() : (
									($it->produto->nome ?? $it->nome ?? '---')
								);

								// Quantidade: cobre 'quantidade' ou 'qtd'
								$qtd  = $it->quantidade ?? $it->qtd ?? 1;

								// Unitário: cobre 'valor_unitario' ou 'valor'
								$unit = $it->valor_unitario ?? $it->valor ?? 0;
