...
<div class="col-md-2">
   {!! Form::tel('cpf_cnpj', 'CPF/CNPJ')
    ->attrs([
        ...
        'id' => 'inp-cpf_cnpj',
        (!__isMaster() && !__isPartialSuperAdmin() ? 'readonly' : null) => true,
        (!__isMaster() && !__isPartialSuperAdmin() ? 'onfocus' : null) => 'this.blur()',
        (!__isMaster() && !__isPartialSuperAdmin() ? 'tabindex' : null) => '-1',
        (!__isMaster() && !__isPartialSuperAdmin() ? 'style' : null) => 'pointer-events: none; background-color: #e9ecef;'
    ])
    ->required()
    ->value(
        old('cpf_cnpj', (__isMaster() || __isPartialSuperAdmin()) ? ($item->cpf_cnpj ?? '') : (Auth::user()->documento ?? ''))
    )
!!}
    ...
    <div class="col-md-3">
        ...
        !!}
         ...
         {{-- Campos ocultos --}}
    {!! Form::hidden('cargo_funcao', Auth::user()->cargo_funcao) !!}
    {!! Form::hidden('atividade', Auth::user()->atividade) !!}
    {!! Form::hidden('qtd_funcionarios', Auth::user()->qtd_funcionarios) !!}
    {!! Form::hidden('nome_contato', Auth::user()->name) !!}
...
@section('js')
<script>
    var isEdit = {{ isset($empresa) ? 'true' : 'false' }};
</script>
...
$(document).ready(function () {
    $('#inp-cpf_cnpj').trigger('blur');
});

$(document).on("blur", "#inp-cpf_cnpj", function () {
    let cpf_cnpj = $(this).val().replace(/[^0-9]/g, ''); // Remove todos os caracteres não numéricos

    if (cpf_cnpj.length == 14) { // Verifica se é CNPJ
        let url = "";

        if (isEdit) {
            // Busca local no banco
            url = path_url + "empresaPorCnpjLocal/" + cpf_cnpj;
        } else {
            // Busca externa API pública
        ...
        $.get(url)
            .done((data) => {
                if (data) {
                    if (isEdit) {
                        // ---- MODO EDIT: preencher com dados locais ----
                        $('#inp-nome').val(data.nome);
                        $('#inp-nome_fantasia').val(data.nome_fantasia);
                        $('#inp-ie').val(data.ie);
                        $('#inp-rua').val(data.rua);
                        $('#inp-numero').val(data.numero);
                        $('#inp-bairro').val(data.bairro);
                        $('#inp-complemento').val(data.complemento);
                        $('#inp-cep').val(data.cep);
                        $('#inp-email').val(data.email);
                        $('#inp-celular').val(data.celular);

                        if (data.cidade) {
                            var newOption = new Option(data.cidade.info, data.cidade_id, false, false);
                            $('#inp-cidade_id').append(newOption).trigger('change');
                        }
                    } else {
                        // ---- MODO CREATE: preencher com dados da API ----
                        $('#inp-ie').val(data.estabelecimento.inscricoes_estaduais.length > 0 
                        ...
                        let telefone = data.estabelecimento.telefone1?.replace(/[^0-9]/g, '') || '';
                        ...
                        $('#inp-celular').val(telefone);
                        $('#inp-celular').mask('(00) 9 0000-0000');
