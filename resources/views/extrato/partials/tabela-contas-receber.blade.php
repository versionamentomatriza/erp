...
<table class="table text-center table-hover">
    ...
    <tbody>
        @forelse($contasReceberPaginadas as $conta)
                ...
                <td>
                    @if ($conta->conciliacoes->isNotEmpty())
                        ...
                        <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal"
                            data-bs-target="#modalDesvincularContasReceber-{{ $conta->id }}" title="Desvincular transações">
                            Desvincular
                        </button>

                        <!-- Botão Vincular -->
                        <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                            data-bs-target="#modalVincularContaReceber-{{ $conta->id }}" title="Vincular a outra transação">
                            Vincular
                        </button>
                    @else
                        <!-- Nenhuma conciliação ainda → só mostrar Vincular -->
                        <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                            data-bs-target="#modalVincularContaReceber-{{ $conta->id }}" title="Vincular a uma transação">
                            Vincular
                        </button>
                    @endif
                </td>
            ...
            @if ($conta->conciliacoes->isNotEmpty())
                <div class="modal fade" id="modalDesvincularContasReceber-{{ $conta->id }}" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal fade" id="modalVincularContaReceber-{{ $conta->id }}" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
