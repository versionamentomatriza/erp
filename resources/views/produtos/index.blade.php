        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="col-lg-12">
                    ...
                    <div class="row mt-3">
                        <div class="col-md-4">
                            {!! Form::text('pesquisa_geral', 'Pesquisar')->placeholder('Pesquisa Geral')->attrs(['class' => 'form-control']) !!}
                        </div>

                        <div class="col-md-2">
                            {!! Form::select('tipo', 'Tipo', ['' => 'Todos', 'composto' => 'Composto', 'variavel' => 'Variável', 'combo' => 'Combo'])->attrs(['class' => 'form-select']) !!}
                        </div>
                ...
                <div class="col-md-12 mt-3 table-responsive">
                    <h6>Total de produtos cadastrados: <strong>{{ $data->total() }}</strong></h6>
                        ...
                        <table class="table table-striped table-centered mb-0">
                            <thead class="table-dark">
                                <tr>
                                    ...
                                    <th></th>
                                    <th>
                                        <x-sortable-column field="nome" label="Nome" />
                                    </th>

                                    <th>Valor de venda</th>
                                    <th>Valor de compra</th>
                                    <th>Estoque Total</th> <!-- NOVA COLUNA ADICIONADA -->
                                    @if(__countLocalAtivo() > 1)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form style="width: 250px" action="{{ route('produtos.destroy', $item->id) }}" method="post" id="form-{{$item->id}}">
                                            ...
                                            <a class="btn btn-light btn-sm" href="{{ route('produtos.etiqueta', [$item->id]) }}" title="Gerar etiqueta" target="_blank">
                                    ...
                                    @endif
                                    <td>
                                        @if(isset($item) && isset($item->unidade)) <!-- ✅ Corrigido -->
                                        @if($item->unidade == 'UN' || $item->unidade == 'UNID')
                                        {{ number_format($item->quantidade_estoque, 0) }}
                                        @else
                                        {{ number_format($item->quantidade_estoque, 3, '.', '') }}
                                        @endif
                                        @else
