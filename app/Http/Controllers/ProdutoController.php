    ...
    public function index(Request $request)
{
    ...
    $pesquisaGeral = $request->get('pesquisa_geral'); 

    // Ordenação só se clicado
    $sort = $request->get('sort'); 
    $direction = $request->get('direction', 'asc');

    $query = Produto::select('produtos.*', \DB::raw('SUM(estoques.quantidade) as quantidade_estoque'))
        ...
        ->when(!empty($pesquisaGeral), function ($q) use ($pesquisaGeral) {
            $q->where(function ($query) use ($pesquisaGeral) {
        ...
        ->when($request->categoria_id, function ($q) use ($request) {
            $q->where('produtos.categoria_id', $request->categoria_id);
        })

        // Tipo
        ->when($tipo, function ($q) use ($tipo) {
            if ($tipo == 'composto') return $q->where('produtos.composto', 1);
            if ($tipo == 'variavel') return $q->whereNotNull('produtos.variacao_modelo_id');
            if ($tipo == 'combo') return $q->where('produtos.combo', 1);
        })

        // Localização
        ->when($local_id, function ($query) use ($local_id) {
            $query->join('produto_localizacaos', 'produto_localizacaos.produto_id', '=', 'produtos.id')
    ...
    // Ordenação: só aplica se tiver sort no request
    if ($sort) {
        $query->orderBy($sort, $direction);
    }

    $data = $query->groupBy('produtos.id')->paginate(env("PAGINACAO"));
    ...
    public function storeModelo(Request $request)
    {
        ...
        // Retorna para a página anterior com reload
    ...
    private function validaArquivo($rows)
    {
        ...
        foreach ($rows[0] as $row) {  // A primeira planilha é sempre o índice 0
