        ...
        try {
            if ($request->empresa) {
                ...
                foreach ($empresas as $e) {
                    if (!$e || !($e instanceof Empresa)) {
                        Log::warning("Registro ignorado: não é uma instância válida de Empresa.");
                        continue;
                    }

                    if (!isset($e->email) || empty($e->email)) {
                        Log::warning("Empresa ID {$e->id} ignorada: email ausente.");
                        continue;
                    }

                    if ($e && method_exists($e, 'getKey') && !__isEmpresaMaster($e)) {

                        try {
                            $requestData = array_merge($request->all(), ['empresa_id' => $e->id]);
                            Notificacao::create($requestData);
                            $total++;
                        } catch (\Exception $ex) {
                            $falhas++;
                            Log::error("Erro ao criar notificação para empresa ID {$e->id}: " . $ex->getMessage());
                        }
                    }
                }
                session()->flash("flash_success", "Notificação criada para $total empresas. Falhas: $falhas");
            }
        } catch (\Exception $e) {
