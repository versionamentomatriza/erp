...
$(".btn-gerar-fatura").on("click", function () {
    ...
    if (total == 0) {
        swal("Erro", "O valor da venda deve ser maior que 0", "error").then(
            () => {
        ...
        data: function (params) {
            ...
            if ($("#inp-local_id").length) {
                ...
                if (!local_id) {
                    swal(
                        "Alerta",
                        "Selecione primeiramente o local",
                        "warning"
                    );
                    return;
                }
                ...
                processResults: function (response) {
                    ...
                    $.each(response, function (i, v) {
                        ...
                        if (compra == 0) {
                            ...
                            o.text +=
                                " R$ " + convertFloatToMoeda(v.valor_compra);
                        }
                        if (v.codigo_barras) {
                            o.text += " [" + v.codigo_barras + "]";
                        }
...
function selecionarVariacao(id, descricao, valor) {
    ...
    $select
        .closest("td")
        .append("<span>variação: <strong>" + descricao + "</strong></span>");
