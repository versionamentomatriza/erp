function transmitir(id){
	...
	.done((success) => {
		swal("Sucesso", "CTe Os emitida " + success.recibo + " - chave: [" + success.chave + "]", "success")
		.then(() => {
...
$('#btn-cancelar').click(() => {
	if(IDCTe != null){
		$.post(path_url + "api/cte_os_painel/cancelar", {
			id: IDCTe,
		...
		.done((success) => {
			...
			.then(() => {
				window.open(path_url + 'cte-os/imprimir-cancela/' + IDCTe, "_blank")
				setTimeout(() => {
					location.reload()
				}, 100)
			})
...
$('#btn-corrigir').click(() => {
	if(IDCTe != null){
		$.post(path_url + "api/cte_os_painel/corrigir", {
			id: IDCTe,
		...
		.done((success) => {
			swal("Sucesso", success, "success")
			.then(() => {
				window.open(path_url + 'cte_os/imprimir-correcao/' + IDCTe, "_blank")
				setTimeout(() => {
					location.reload()
				}, 100)
			})
...
function consultar(id, numero){
	$.post(path_url + "api/cte_os_painel/consultar", {
		id: id,
	})
	.done((success) => {
		swal("Sucesso", success, "success")
